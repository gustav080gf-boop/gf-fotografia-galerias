...
function getGalleryPublic_(slug) {
  ...
  if (!g) return { ok: false, error: 'Galería no encontrada.' };
  if (!isGalleryAvailable_(g)) return { ok: false, error: 'La galería no está disponible.' };
  return {
    ok: true,
    gallery: {
      id: g.ID,
...
function authenticate_(slug, pin) {
  const g = findGallery_(slug);
  if (!g || !isGalleryAvailable_(g)) return { ok: false, error: 'Galería no disponible.' };
  pin = String(pin || '').trim();
  if (!/^\d{4}$/.test(pin)) return { ok: false, error: 'Clave inválida.' };
  ...
  return { ok: true, token: token, expiresIn: GF.TOKEN_TTL_SECONDS };
}
...
function getPhotos_(token, page) {
  ...
  while (files.hasNext()) {
    const f = files.next();
    if (/^image\//i.test(f.getMimeType())) all.push({ id: f.getId(), name: f.getName(), mime: f.getMimeType() });
  }
  ...
  const photos = slice.map((f, i) => ({
    ...
    filename: f.name,
    preview: getPreviewDataUrl_(f.id, GF.PREVIEW_SIZE),
...
function getPreviewDataUrl_(fileId, width) {
  ...
  const metaResp = UrlFetchApp.fetch(metaUrl, { headers: { Authorization: 'Bearer ' + token }, muteHttpExceptions: true });
